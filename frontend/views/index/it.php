...
<article>
    <div class="blogs">
        <?php foreach ($models as $model): ?>
            <li>
                <h3 class="blogtitle" style="height: 40px"><a href="<?=\yii\helpers\Url::to(['index/read','id'=>$model->id])?>"><?=$model->title?></a></h3>
                <div class="bloginfo">
                    <p><?=$model->intro?></p>
                </div>
                <div class="autor"><span class="lm"><a href="/" title="<?=$model->categroy->name?>" target="_blank" class="classname"><?=$model->categroy->name?></a></span><span class="dtime"><?=date('Y-m-d',$model->created_time)?></span><span class="viewnum">浏览（<a href="/"><?=$model->click?></a>）</span><span class="readmore"><a href="<?=\yii\helpers\Url::to(['index/read','id'=>$model->id])?>">阅读原文</a></span></div>
            </li>
        <?php endforeach;?>
    </div>
